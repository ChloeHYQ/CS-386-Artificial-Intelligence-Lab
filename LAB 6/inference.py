...
class ParticleFilter(InferenceModule):
	...
	def initializeUniformly(self, gameState):
		...
		temp_list = [pos for pos in self.legalPositions]
		while num_added < self.numParticles:
			self.particles.extend(temp_list)
			num_added += numLegalpos
	...
	def observe(self, observation, gameState):
		...
		if noisyDistance != None:
			...
			for pos in self.legalPositions:
				self.weight[pos] *= pos_count[pos]
				sum_weights += self.weight[pos]
			...
			for pos in self.legalPositions:
				self.weight[pos] /= (sum_weights*1.0)
			
			new_particles = []
			for i in range(self.numParticles):
				temp_sample = util.sample(self.weight)
				new_particles.append(temp_sample)
			...
			self.particles = [jailpos for i in range(self.numParticles)]
...
class JointParticleFilter:
	...
	def initializeParticles(self):
		...
		num_added = 0
		self.particles = []
		temp_list = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
		numLegalpos = len(temp_list)
	...
	def observeState(self, gameState):
		...
		for i in range(len(self.particles)):
			for j in range(self.numGhosts):
				if noisyDistances[j] == None:
					temp_particle = self.getParticleWithGhostInJail(self.particles[i],j)
					self.particles[i] = temp_particle
			self.weight[self.particles[i]] = 0
		...
		for particle in self.particles:
			...
			for i in range(self.numGhosts):
				if noisyDistances[i] != None:
					...
					temp_weight *= emissionModels[i][trueDistance]
			self.weight[particle] += temp_weight

		tot_count = self.weight.totalCount()
		...
		for particle in self.weight:
			self.weight[particle]/=tot_count
	...
	def elapseTime(self, gameState):
		...
		for oldParticle in self.particles:
			...
			for i in range(self.numGhosts):
				...
				updated_list.append(util.sample(newPosDist))
			newParticle = updated_list
			"*** END YOUR CODE HERE ***"
	...
	def getBeliefDistribution(self):
		...
		self.beliefs = util.Counter()
		tot_list = []
		for ghost_agent in self.ghostAgents:
			pos_list = self.legalPositions[:]
			pos_list.append(self.getJailPosition(ghost_agent.index))
			tot_list.append(pos_list)

		pos_tuple_list = list(itertools.product(*tot_list))
		
		for pos in pos_tuple_list:
			self.beliefs[pos] = 0
